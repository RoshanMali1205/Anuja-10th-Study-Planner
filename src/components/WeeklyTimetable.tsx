...
const schedule: DaySchedule[] = [
  ...
  },
  {
    ...
    morning: { subject: 'Revision', icon: '🔄', color: '#FF6B35' },
    evening: { subject: 'Revision', icon: '🔄', color: '#FF6B35' },
  },
]
  ...
  return (
    <div>
      <h2 className="font-heading font-bold text-lg text-[#1A1A2E] dark:text-white flex items-center gap-2 mb-3">
        <span className="w-8 h-8 rounded-lg bg-[#E8DAEF] flex items-center justify-center text-sm">🗓️</span>
        Weekly Study Timetable
      </h2>

      <div className="warm-card rounded-2xl overflow-hidden mb-6">
        {/* Header */}
        <div className="grid grid-cols-[1fr_1.5fr_1.5fr] bg-gradient-to-r from-[#FF6B35] to-[#E85D25] text-white text-xs font-heading font-bold">
        ...
        <div className="grid grid-cols-[1fr_1.5fr_1.5fr] sm:hidden text-[0.6rem] text-gray-400 dark:text-gray-500 font-medium bg-gray-50 dark:bg-[#1e2237]">
          ...
          <div className="px-3 py-1 border-l border-[#FFE0CC] dark:border-gray-700">6:00 – 8:00 AM</div>
          <div className="px-3 py-1 border-l border-[#FFE0CC] dark:border-gray-700">5:00 – 7:00 PM</div>
        </div>
          ...
          return (
            <div
              ...
              className={`grid grid-cols-[1fr_1.5fr_1.5fr] border-t text-sm ${
                isToday
                  ? 'bg-[#FFF3E0] dark:bg-[#3d2e1a] border-t-[#FFB347]'
                  : 'border-t-[#FFE0CC] dark:border-t-gray-700 even:bg-[#FFFCFA] dark:even:bg-[#1e2237]'
              }`}
            >
              {/* Day column */}
              <div className={`px-3 py-2.5 font-heading font-bold text-[0.82rem] flex items-center gap-1.5 ${
                isToday ? 'text-[#FF6B35]' : 'text-gray-700 dark:text-gray-300'
              }`}>
                <span className="hidden sm:inline">{day.day}</span>
                <span className="sm:hidden">{day.shortDay}</span>
                {isToday && (
                  <span className="text-[0.55rem] bg-[#FF6B35] text-white px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">
                    Today
                  </span>
                )}
              </div>
              ...
              {/* Morning */}
              <div className="px-3 py-2.5 border-l border-[#FFE0CC] dark:border-gray-700 flex items-center gap-2">
        ...
        {/* Footer note */}
        <div className="px-3 py-2.5 bg-[#FFF8F0] dark:bg-[#2a2540] border-t border-[#FFE0CC] dark:border-gray-700 text-[0.72rem] text-gray-500 dark:text-gray-400 text-center">
